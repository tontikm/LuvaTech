...
export async function sendQuoteEmail(
  ...
  const deliverablesHtml = quote.deliverables.map((d) => `<li>${d}</li>`).join("");
  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 560px; color: #111;">
      ...
      <h1 style="font-size: 24px; font-weight: 600; margin: 0 0 8px;">Hi ${quote.contactName},</h1>
      <p style="color: #444; line-height: 1.6;">Thank you for speaking with our AI assistant. Your project quotation <strong>${quote.quoteNumber}</strong> for <strong>${quote.businessName}</strong> is attached.</p>
      <div style="background: #f7f7f7; border-radius: 12px; padding: 20px; margin: 24px 0;">
        ...
        <p style="margin: 8px 0 0; color: #666; font-size: 14px;">Timeline: ${quote.estimatedTimeline}</p>
      </div>
      <h2 style="font-size: 16px; margin-bottom: 8px;">Project summary</h2>
      <p style="color: #444; line-height: 1.6;">${quote.projectSummary}</p>
  ...
  await sendEmail({
    ...
    html: `<p>New quote <strong>${quote.quoteNumber}</strong> for ${quote.businessName} (${quote.email}): ${formatCurrency(quote.priceEstimate)}</p>`,
  });
  ...
  return sendEmail({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 520px;">
        <h1 style="font-size: 22px;">You're booked, ${input.name}</h1>
        ...
        ${input.company ? `<p style="color: #666;">Company: ${input.company}</p>` : ""}
      </div>
    `,
  });
}
